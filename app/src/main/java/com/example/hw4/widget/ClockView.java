package com.example.hw4.widget;
...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float hourEndX = (float)(centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree - 90)));
        float hourEndY = (float)(centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree - 90)));
        float minuteEndX = (float)(centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minuteDegree - 90)));
        float minuteEndY = (float)(centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minuteDegree - 90)));
        float secondEndX = (float)(centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secondDegree - 90)));
        float secondEndY = (float)(centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secondDegree - 90)));
    ...
    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        for (int i = 0; i < 12; i++) {
            float posX = (float)(centerX + radius * 0.8f * Math.cos(Math.toRadians(30 * i - 90)));
            float posY = (float)(centerY + radius * 0.8f * Math.sin(Math.toRadians(30 * i - 90)));
            Paint.FontMetrics fontMetrics=numberPaint.getFontMetrics();
            float distance=(fontMetrics.bottom - fontMetrics.top)/2 - fontMetrics.bottom;
            canvas.drawText(String.valueOf(i), posX, posY + distance, numberPaint);
        }
    }
